...
SELECT x.title AS title, y.inventory_id AS inventory_id 
FROM (
...
SELECT y.title, SUM(x.times_rented) AS 'Times Rented' 
FROM (
...
SELECT iden.store_id, loc.Location 
FROM (
	SELECT z.city_id, CONCAT(z.city,", ", v.country) AS Location 
    FROM (
...
JOIN (
    ...
    FROM address
	INNER JOIN store 
    ON address.address_id=store.address_id) AS iden
ON iden.city_id = loc.city_id;

-- 7h ----------------------------------------------------------------------------

/*Display total amount per genre*/
SELECT SUM(x.amount) AS Total, y.name As Category 
...
LEFT JOIN (
	SELECT i.title, f.name, i.inventory_id 
    FROM (
